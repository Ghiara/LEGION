...
class QFunction(base_component.Component):
    def __init__(
        ...
        Args:
            obs_dim (int): size of the observation. TODO check obs_dim
    ...
    def forward(self, mtobs: MTObs) -> TensorType:
        ...
        if self.should_condition_model_on_task_info: # false
...
class Critic(base_component.Component):
    def __init__(
        ...
        if key in encoder_cfg:
            ...
            encoder_cfg = encoder_cfg[encoder_type_to_select] # encoder vae
        ...
        if self.should_use_multi_head_policy: # False
    ...
    def forward(
        ...
        obs_action = torch.cat([obs, action], dim=-1) # concat latent obs with action
