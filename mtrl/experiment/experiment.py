...
class Experiment(checkpointable.Checkpointable):
    # checkpoint table --> abstract save(),load()
    def __init__(self, config: ConfigType, experiment_id: str = "0"):
        ...
        # rewrite ordered_task_list in form of dict(name:index)
        ...
        # resume saved agent & replay buffer
        ...
        # Log name to idx map
        self.logger.log_text('train/Name_to_index', self.task_name_to_idx, 0)
...
def get_env_metadata(
    ...
    metadata: EnvMetaDataType = {
        # "env_obs_space": dummy_env.observation_space if not config.env.use_onehot else dummy_env.observation_space+len(env.env_fns),
